...
//
//Row Reduction Method (Incomplete)
//

void rowReduction(double** matrix)
{
	int counter = 0;
	
	//will loop this either recursively or with while/for/whatever
	double one = matrix[counter][0];
	double two = matrix[counter + 1][0];
	double multiple = one / two;
	
	//multiply all of row two by arbitrary multiple
	for (int i = 0; i < ARRAYSIZE; i++) 
	{
		matrix[counter + 1][i] *= multiple;
	}
	
	//subtract lower row from upper row
	for (int i = 0; i < ARRAYSIZE; i++)
	{
		matrix[counter+1][i] -= matrix[counter][i];
	}
...
int main(int argc, char *argv[])
{
	...
	//
	//calling the row reduction method
	//
